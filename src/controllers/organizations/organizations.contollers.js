...
const OrganizationController = {
  ...
  update: async (req, res) => {
    ...
    if (!bookData.organization_id) {
      return res.status(400).json({ error: "Organization ID is required for update" });
    }
  ...
  delete: async (req,res)=>{
    ...
    //  console.log("Book ID to delete:", organization_id);
     ...
     return res.status(200).json({ message: "Organization deleted successfully", organization_id });
  }
};
